...
public class OrgEfficiencyAnalysisService {
    ...
    public Map<String, Object> getOrganizationHealth() {
        ...
        Double stabilityRate = queryForObjectWithLatestDt(hiveJdbc, stabilitySql, latestDt);
        ...
        String turnoverSql = "SELECT " +
                ...
                "FROM dim_employee " + buildLatestDtWhereClause(latestDt);
        Double turnoverRate = queryForObjectWithLatestDt(hiveJdbc, turnoverSql, latestDt);
        result.put("turnoverRate", turnoverRate);
        
        // 薪酬竞争力（平均薪资）
        String salarySql = "SELECT AVG(current_salary) as avg_salary FROM dim_employee " + buildLatestDtWhereClause(latestDt);
        Double avgSalary = queryForObjectWithLatestDt(hiveJdbc, salarySql, latestDt);
        ...
        String eduSql = "SELECT " +
                ...
                "FROM dim_employee " + buildLatestDtWhereClause(latestDt);
        Double highEduRate = queryForObjectWithLatestDt(hiveJdbc, eduSql, latestDt);
