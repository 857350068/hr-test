...
public class PerformanceEvaluationService extends ServiceImpl<PerformanceEvaluationMapper, PerformanceEvaluation> {
    ...
    public Map<String, Object> getImprovementOverview() {
        Map<String, Object> result = new HashMap<>();
        long total = this.count();
        long completed = this.count(new LambdaQueryWrapper<PerformanceEvaluation>()
                .eq(PerformanceEvaluation::getEvaluationStatus, 3));
        long needImprove = this.count(new LambdaQueryWrapper<PerformanceEvaluation>()
                .in(PerformanceEvaluation::getPerformanceLevel, "C", "D"));
        long withPlan = this.count(new LambdaQueryWrapper<PerformanceEvaluation>()
                .isNotNull(PerformanceEvaluation::getImprovementPlan));
        long withInterview = this.count(new LambdaQueryWrapper<PerformanceEvaluation>()
                .isNotNull(PerformanceEvaluation::getInterviewRecord));
        double avgFinalScore = this.baseMapper.selectObjs(new LambdaQueryWrapper<PerformanceEvaluation>()
                        .select(PerformanceEvaluation::getFinalScore)
                        .isNotNull(PerformanceEvaluation::getFinalScore))
                .stream()
        ...
        result.put("needImproveCount", needImprove);
        result.put("improvementPlanCount", withPlan);
        result.put("interviewCount", withInterview);
        result.put("avgFinalScore", Math.round(avgFinalScore * 100.0) / 100.0);
        return result;
    }
}
